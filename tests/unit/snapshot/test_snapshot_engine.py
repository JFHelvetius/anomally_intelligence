...
def _w_and_t(tmp_path: Path, archive_root: Path):
    ...
    t = build_timeline(archive_root=archive_root, workspace=w, timeline_id="tl")
    return w, t
...
def test_snapshot_reference_constructs() -> None:
    r = SnapshotReference(reference_type="evidence", identifier="E1", artifact_hash="a" * 64)
...
def test_snapshot_constructs(tmp_path: Path, archive_root: Path) -> None:
...
def test_snapshot_frozen(tmp_path: Path, archive_root: Path) -> None:
    w, t = _w_and_t(tmp_path, archive_root)
    s = create_snapshot(snapshot_id="s1", workspace=w, timeline=t)
...
def test_snapshot_is_deterministic_across_runs(tmp_path: Path, archive_root: Path) -> None:
    w, t = _w_and_t(tmp_path, archive_root)
...
def test_verify_snapshot_success(tmp_path: Path, archive_root: Path) -> None:
    w, t = _w_and_t(tmp_path, archive_root)
...
def test_compute_snapshot_hash_matches_stored(tmp_path: Path, archive_root: Path) -> None:
    w, t = _w_and_t(tmp_path, archive_root)
    s = create_snapshot(snapshot_id="s", workspace=w, timeline=t)
...
def test_snapshot_does_not_modify_archive_manifest(tmp_path: Path, archive_root: Path) -> None:
    w, t = _w_and_t(tmp_path, archive_root)
...
def test_snapshot_imports_no_forbidden_engines() -> None:
    ...
    assert offenders == [], f"snapshot/ imports forbidden engines: {offenders}"
...
def test_no_prohibited_tokens_in_snapshot_module() -> None:
    ...
    assert offenders == [], f"Forbidden tokens in snapshot: {offenders}"
