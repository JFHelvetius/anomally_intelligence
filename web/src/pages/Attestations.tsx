...
import { Card, CardHeader, Hash, Badge, PageHeader, EmptyState, Skeleton } from '../components/ui'
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-violet-50 border border-violet-200 rounded-xl px-5 py-4 flex items-start gap-3">
        <Shield size={16} className="text-violet-700 mt-0.5 shrink-0" />
      ...
      {isError && <div className="text-red-700 text-sm">Failed to load attestations.</div>}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="text-sm text-slate-800 font-mono">{a.id}</span>
...
export function AttestationDetail() {
  ...
  if (isError) return (
    <div className="max-w-3xl mx-auto space-y-4">
      <Link to="/attestations" className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-violet-700">
        <ArrowLeft size={12} /> Back
      </Link>
      <div className="text-red-700 text-sm">Attestation not found.</div>
    </div>
  )
  if (!data) return null
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
      <Link to="/attestations" className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-violet-700 transition-colors">
        <ArrowLeft size={12} /> Back to Attestations
      </Link>

      {/* Header card */}
      <div className="bg-[var(--surface)] border border-orange-200 rounded-xl p-5">
        <div className="flex items-start justify-between gap-4 mb-4">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-1 text-xs text-slate-500 hover:text-violet-700"
          >
            Raw JSON <ExternalLink size={11} />
          </a>
        </div>
        <p className="text-lg font-bold text-slate-900 font-mono mb-2">{id}</p>
      ...
      <Card className="divide-y divide-[#1e2535]">
        {[
          ...
          <div key={label as string} className="grid grid-cols-[160px_1fr] gap-3 px-5 py-3 items-start">
            <span className="text-[11px] text-slate-500 uppercase tracking-wider pt-0.5">{label}</span>
            <span className="text-xs font-mono text-slate-700 break-all">{value as string}</span>
          </div>
        ))}
      </Card>
      ...
      {data.signature && (
        <Card>
          ...
          <div className="px-5 py-4">
            <code className="text-[11px] font-mono text-slate-400 break-all leading-relaxed">
